...
class ChangePointPredictor:
    ...
    def prepare_training_data(
        ...
        martingales: Dict[str, np.ndarray]
        ...
        if martingales:
            ...
            for m in martingales.values():
                ...
                m_array = np.array([float(x.item()) if isinstance(x, np.ndarray) else float(x) for x in m])
            ...
            for i in range(len(features) - self.config.window_size - self.config.forecast_horizon + 1):
                ...
                target_window = stacked_martingales[
                    ...
                    i + self.config.window_size + self.config.forecast_horizon
                ]
                # Expand target to match model output dimensions
                target_expanded = np.tile(target_window[:, np.newaxis, :], (1, n_nodes, 1))
        ...
        if y:
            y = torch.FloatTensor(np.array(y))  # [batch, forecast_horizon, n_nodes, n_features]
        else:
            y = torch.FloatTensor()
    ...
    def predict(
        self,
        current_graphs: List[np.ndarray],
...
def main():
    ...
    for t in range(40, len(graphs)-predictor.config.window_size):
        ...
        if predictions["earliest_detection"] is not None:
            logger.info(f"Change predicted in {predictions['earliest_detection']} steps")
            logger.info(f"Confidence: {predictions['max_confidence']:.2f}")
            logger.info(f"Predicted martingale sequence: {pred_info['martingale_values'].flatten()}")
        ...
        if len(predictions_log) >= 2:
            ...
            if curr_max > prev_max * 1.5:  # 50% increase
                ...
                logger.warning(f"Previous max: {prev_max:.4f}, Current max: {curr_max:.4f}")
