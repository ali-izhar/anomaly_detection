# tests/visualize_nw_martingales.py

"""
Newman-Watts Graph Martingale Visualization Script

This script demonstrates the martingale-based change point detection for NW graphs.
It generates NW graphs, computes martingales, and visualizes the results.

Usage:
    1. Update the NW_MARTINGALE_CONFIG if needed
    2. Run the script: python tests/visualize_nw_martingales.py
"""
...
@dataclass
class NWMartingaleAnalyzer:
    """Analyzer for NW graph martingales."""

    threshold: float = NW_MARTINGALE_CONFIG["threshold"]
    epsilon: float = NW_MARTINGALE_CONFIG["epsilon"]
    output_dir: str = NW_MARTINGALE_CONFIG["output_dir"]
...
def main():
    """Main entry point for NW graph martingale analysis."""
    print("\nStarting NW Graph Martingale Analysis")
    print("----------------------------------------")
    print(f"Configuration:")
    print(f"  - Threshold: {NW_MARTINGALE_CONFIG['threshold']}")
    print(f"  - Epsilon: {NW_MARTINGALE_CONFIG['epsilon']}")
    print(f"  - Output directory: {NW_MARTINGALE_CONFIG['output_dir']}")

    # Create analyzer and run analysis
    analyzer = NWMartingaleAnalyzer()
