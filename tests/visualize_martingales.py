...
class MartingaleVisualizer:
    ...
    def save_results(self) -> None:
        ...
        with open(self.output_dir / f"{self.graph_type.lower()}_martingale_results.txt", "w") as f:
    ...
    def _plot_martingale_sequence(
        ...
        if cumulative:
            ax.semilogy(M_avg, color="#FF4B4B", label="Average", linewidth=2.5, alpha=0.9)
            ax.semilogy(M_sum, color="#2F2F2F", label="Sum", linewidth=2.5, linestyle="-.", alpha=0.8)
        else:
            ax.plot(M_avg, color="#FF4B4B", label="Average", linewidth=2.5, alpha=0.9)
            ax.plot(M_sum, color="#2F2F2F", label="Sum", linewidth=2.5, linestyle="-.", alpha=0.8)
    ...
    def _customize_martingale_plot(
        ...
        ax.set_ylabel("Martingale Values" + (" (log scale)" if cumulative else ""), fontsize=12, labelpad=10)
