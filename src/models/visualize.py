    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            for feat_name in predictions:
                ...
                plt.tight_layout()
                plt.savefig(
                    save_dir / f"sample_{batch_idx}_{feat_name}.png",
                    bbox_inches="tight",
                    dpi=300,
                )
                plt.close()
                ...
                plt.grid(True)
                plt.savefig(
                    save_dir / f"evolution_{batch_idx}_{feat_name}.png",
                    bbox_inches="tight",
                    dpi=300,
                )
                plt.close()
...
def plot_loss_curves(tensorboard_path: Path, save_dir: Path):
    ...
    train_steps, train_values = zip(
        *[(s.step, s.value) for s in event_acc.Scalars("Loss/train")]
    )
    val_steps, val_values = zip(
        *[(s.step, s.value) for s in event_acc.Scalars("Loss/val")]
    )
