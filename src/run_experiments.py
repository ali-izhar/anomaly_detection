...
def run_experiment(exp_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "metrics": metrics, "exp_name": exp_name}
    except Exception as e:
        logger.error(f"Error running experiment {exp_name}: {str(e)}")
        return {"success": False, "error": str(e), "exp_name": exp_name}
...
def main():
    ...
    if args.all or args.sensitivity:
        ...
        run_sensitivity_experiments(max_workers=args.workers)

    if args.all or args.comparison:
        logger.info("Running comparison experiments (Table IV)")
        run_comparison_experiments(max_workers=args.workers)
