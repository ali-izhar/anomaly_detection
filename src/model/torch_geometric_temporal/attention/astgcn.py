...
class ChebConvAttention(MessagePassing):
    ...
    def __norm__(
        ...
        return (
            edge_index,
            edge_weight,
        )  # for example 307 nodes as deg, 340 edges , 307 nodes as self connections
    ...
    def forward(
        ...
        row, col = (
            edge_index  # refer to the index of each note each is a list of nodes not a number # (954, 954)
        )
        Att_norm = (
            norm * spatial_attention[:, row, col]
        )  # spatial_attention for example (32, 307, 307), -> (954) * (32, 954) -> (32, 954)
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, in_channels: int, num_of_vertices: int, num_of_timesteps: int):
        ...
        self._W2 = nn.Parameter(
            torch.FloatTensor(in_channels, num_of_timesteps)
        )  # for example (1, 12)
        self._W3 = nn.Parameter(torch.FloatTensor(in_channels))  # for example (1)
        self._bs = nn.Parameter(
            torch.FloatTensor(1, num_of_vertices, num_of_vertices)
        )  # for example (1,307, 307)
        self._Vs = nn.Parameter(
            torch.FloatTensor(num_of_vertices, num_of_vertices)
        )  # for example (307, 307)
...
class TemporalAttention(nn.Module):
    ...
    def __init__(self, in_channels: int, num_of_vertices: int, num_of_timesteps: int):
        ...
        self._U2 = nn.Parameter(
            torch.FloatTensor(in_channels, num_of_vertices)
        )  # for example (1, 307)
        ...
        self._Ve = nn.Parameter(
            torch.FloatTensor(num_of_timesteps, num_of_timesteps)
        )  # for example (12, 12)
    ...
    def forward(self, X: torch.FloatTensor) -> torch.FloatTensor:
        ...
        LHS = torch.matmul(
            torch.matmul(X.permute(0, 3, 2, 1), self._U1), self._U2
        )  # (32, 12, 307)
...
class ASTGCNBlock(nn.Module):
    ...
    def forward(
        ...
        X_hat = self._time_convolution(
            X_hat.permute(0, 2, 1, 3)
        )  # will give (32, 64, 307,12)
        # (b,N,F,T)-permute>(b,F,N,T) (1,1)->(b,F,N,T)  (32, 64, 307, 12)
        X = self._residual_convolution(
            X.permute(0, 2, 1, 3)
        )  # will also give (32, 64, 307,12)
