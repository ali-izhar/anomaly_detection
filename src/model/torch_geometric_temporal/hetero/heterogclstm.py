...
class HeteroGCLSTM(torch.nn.Module):
    ...
    def _create_input_gate_parameters_and_layers(self):
        self.conv_i = HeteroConv(
            {
                edge_type: SAGEConv(
                    in_channels=(-1, -1), out_channels=self.out_channels, bias=self.bias
                )
    ...
    def _calculate_cell_state(
        ...
        c_dict = {
            node_type: f_dict[node_type] * C + i_dict[node_type] * t_dict[node_type]
    ...
    def forward(
        ...
        c_dict = self._calculate_cell_state(
            x_dict, edge_index_dict, h_dict, c_dict, i_dict, f_dict
        )
