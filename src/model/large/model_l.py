...
class TemporalConvBlock(nn.Module):
    ...
    def __init__(self, channels, kernel_sizes=[3, 5, 7]):
        ...
        self.out_channels_last = channels - (
            self.out_channels_per_conv * (len(kernel_sizes) - 1)
        )
        ...
        for i, k in enumerate(kernel_sizes):
            out_channels = (
                self.out_channels_last
                if i == len(kernel_sizes) - 1
                else self.out_channels_per_conv
            )
            self.convs.append(
...
class LargeGMAN(nn.Module):
    def __init__(
        ...
        self.structure_blocks = nn.ModuleList(
            [StructuralBlock(hidden_channels, spatial_heads) for _ in range(num_layers)]
        )
        ...
        self.edge_predictor = nn.Sequential(
            ...
            ),  # Remove bias for better initialization
        )
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        temporal_features = temporal_features.transpose(
            1, 2
        ...
        logits = torch.stack(edge_logits, dim=1).view(
        ...
        self.regularization_loss = (
            ...
            + 0.005
            * torch.abs(
                adj_probs - torch.matmul(adj_probs, adj_probs)
            ).mean()  # Reduced from 0.01
        )
