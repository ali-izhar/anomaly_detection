...
def plot_network_specific_analysis(
    ...
    if "epsilon" in df.columns and "trad_tpr" in df.columns:
        best_epsilon = df.groupby("epsilon")["trad_tpr"].mean().idxmax()
        annotations["best_params"]["epsilon"] = best_epsilon
    ...
    annotations["metrics"]["avg_missed_rate"] = f"{(1 - df['trad_tpr'].mean()):.4f}"
    ...
    # Create epsilon effect visualization
    if "epsilon" in df.columns:
        annotations["plot_annotations"]["epsilon_effect"] = [
            "Impact of epsilon parameter on true positive rate",
            "Effect of epsilon on detection delay",
            "Analysis by betting function",
        ]
        plot_epsilon_effect(
            df, output_dir, network_name, annotations, paper_ready=paper_ready
        )

    # Create detection metrics tradeoff visualization
    annotations["plot_annotations"]["detection_metrics"] = [
        "TPR vs. FPR tradeoff analysis with threshold influence",
        "Epsilon parameter effect on true positive rate",
        "Threshold effect on false positive rate with theoretical bound",
        "Window size impact on missed detection rate",
    ]
    plot_detection_metrics_tradeoff(
        df, output_dir, network_name, annotations, paper_ready=paper_ready
    )
...
def plot_betting_function_comparison(
    ...
    # Rename the column to avoid 'betting_func' appearing in the plot
    if paper_ready:
        plot_df = plot_df.rename(columns={"betting_func": " "})
        x_col = " "
    else:
        x_col = "betting_func"
    ...
    # Create the more aesthetically pleasing boxplot
    ...
    # Set the x-tick labels after the plot is created
    if paper_ready:
        # Get current labels first
        labels = [item.get_text() for item in ax.get_xticklabels()]
        # Now set them capitalized
        ax.set_xticks(ax.get_xticks())
        ax.set_xticklabels([l.capitalize() for l in labels])
        # Remove the "betting_func" label that appears at the bottom
    ...
    if paper_ready:
        ...
        sns.stripplot(
            x="betting_func" if x_col == "betting_func" else x_col,
            y="trad_tpr",
            data=plot_df,
            ax=ax,
            color="black",
    ...
    apply_style_to_axes(
        ...
        xlabel=(
            "" if paper_ready else "Betting Function"
        ),  # Remove x-label in paper mode
        ylabel="True Positive Rate",
        legend=False,
        paper_ready=paper_ready,
    )
    ...
    # Save the figure
    if paper_ready:
        # Final pass to ensure any secondary labels are removed
        for i in range(3):
            ax = axs[i]
            ax.set_xlabel("")
            # Remove any labels that seaborn might have added
            if hasattr(ax, "texts"):
                for text in ax.texts:
                    if text.get_text() == "betting_func" or text.get_text() == " ":
                        text.set_visible(False)
            # Make sure there's no x-axis label
            ax.xaxis.label.set_visible(False)
...
def plot_epsilon_effect(
    ...
    """Create visualizations showing the effect of epsilon parameter on detection performance.

    Args:
        df: DataFrame containing analysis results
        output_dir: Directory to save plots
        network_name: Network type (for single-network analysis)
        annotations: Dictionary for storing analysis insights
        paper_ready: Whether to create publication-ready plots
    """
    if "epsilon" not in df.columns:
        print("Warning: Epsilon parameter not found in data")
        return
    ...
    if annotations is None:
        annotations = {
            ...
            "best_params": {},
            "metrics": {},
            "plot_annotations": {"epsilon_effect": []},
        }

    # Create figure with two subplots side by side
    if paper_ready:
        fig, axs = plt.subplots(1, 2, figsize=(7.5, 3.8), constrained_layout=True)
    else:
    ...
    if not paper_ready:
        fig.suptitle(
            f"Impact of Epsilon Parameter on {'All Networks' if network_name is None else NETWORK_NAMES.get(network_name, network_name.upper()) + ' Network'} Performance",
            fontsize=16,
            fontweight="bold",
        )

    # Plot 1: Epsilon vs. TPR
    ax = axs[0]

    if network_name is None:
        # Comparative analysis across networks
        for network in df["network"].unique():
            subnet = df[df["network"] == network]
            epsilon_groups = (
                subnet.groupby("epsilon", observed=True)["trad_tpr"]
                .mean()
                .reset_index()
            )

            if not epsilon_groups.empty:
                markersize = 4 if paper_ready else 7
                linewidth = 1.0 if paper_ready else 2.0
                ax.plot(
                    epsilon_groups["epsilon"],
                    epsilon_groups["trad_tpr"],
                    "o-",
                    label=NETWORK_NAMES.get(network, network.upper()),
        ...
        # Single network analysis by betting function
        betting_funcs = sorted(df["betting_func"].unique())

        for bf in betting_funcs:
            subnet = df[df["betting_func"] == bf]
            epsilon_groups = (
                subnet.groupby("epsilon", observed=True)["trad_tpr"]
                .mean()
                .reset_index()
            ...
            if not epsilon_groups.empty:
                ...
                ax.plot(
                    ...
                    "o-",
                    label=bf.capitalize(),
                    color=BETTING_COLORS.get(bf, "gray"),
                    linewidth=linewidth,
                    markersize=markersize,
                    alpha=0.8,
                )
    ...
    apply_style_to_axes(
        ax,
        title="A. Epsilon vs. True Positive Rate",
    ...
    # Plot 2: Epsilon vs. Detection Delay
    ax = axs[1]
    ...
    if "traditional_avg_delay" in df.columns:
        if network_name is None:
            ...
            for network in df["network"].unique():
                subnet = df[df["network"] == network]
                epsilon_groups = (
                    subnet.groupby("epsilon", observed=True)["traditional_avg_delay"]
                    .mean()
                    .reset_index()
                )
                ...
                if not epsilon_groups.empty:
                    ...
                    ax.plot(
                        epsilon_groups["epsilon"],
                        epsilon_groups["traditional_avg_delay"],
                        "o-",
                        label=NETWORK_NAMES.get(network, network.upper()),
                        color=NETWORK_COLORS.get(network, "gray"),
                        linewidth=linewidth,
        ...
        apply_style_to_axes(
            ax,
            title="B. Epsilon vs. Detection Delay",
            xlabel=r"Epsilon ($\epsilon$)",
            ylabel="Detection Delay (timesteps)",
            paper_ready=paper_ready,
        )

    # Find the best epsilon value
    if "trad_tpr" in df.columns:
        best_epsilon = df.groupby("epsilon")["trad_tpr"].mean().idxmax()
        annotations["best_params"]["best_epsilon"] = best_epsilon

        # Add annotation only if not paper_ready
        if not paper_ready:
            textstr = f"Best epsilon value: {best_epsilon}\n\n"
            textstr += f"• Higher values tend to make betting more aggressive\n"
            textstr += f"• Lower values are more conservative\n"
            textstr += f"• Optimal setting depends on network type and betting function"
            ...
            annotations["plot_annotations"]["epsilon_effect"].append(
                f"Optimal epsilon value identified as {best_epsilon}"
            )
    ...
    if paper_ready:
        output_path = os.path.join(
            output_dir,
            f"{'comparative' if network_name is None else network_name}_epsilon_effect_paper.png",
        )
    else:
        output_path = os.path.join(
            output_dir,
            f"{'comparative' if network_name is None else network_name}_epsilon_effect.png",
        )

    plt.savefig(output_path, dpi=300, bbox_inches="tight")

    # Also save as PDF for paper-ready plots
    if paper_ready:
    ...
    print(f"Epsilon parameter analysis saved to {output_path}")


def plot_detection_metrics_tradeoff(
    df: pd.DataFrame,
    output_dir: str,
    network_name: str = None,
    annotations: Dict = None,
    ...
    """Create a comprehensive visualization of detection metrics tradeoffs.

    This plot shows the relationship between TPR, FPR, and missed detection rates
    ...
    Args:
        ...
        paper_ready: Whether to create publication-ready plots
    """
    if "trad_tpr" not in df.columns or "trad_fpr" not in df.columns:
        print("Warning: Required metrics not found in data")
        return
    ...
    if annotations is None:
        annotations = {
            ...
            "best_params": {},
            "metrics": {},
            "plot_annotations": {"detection_metrics": []},
        }

    # Calculate missed detection rate (1 - TPR)
    df = df.copy()
    df["missed_detection_rate"] = 1 - df["trad_tpr"]

    # Create figure with 2x2 grid for metrics visualization
    if paper_ready:
        fig = plt.figure(figsize=(7.5, 7), constrained_layout=True)
    ...
    if not paper_ready:
        fig.suptitle(
            f"Detection Metrics Analysis for {'All Networks' if network_name is None else NETWORK_NAMES.get(network_name, network_name.upper()) + ' Network'}",
            fontsize=18,
            fontweight="bold",
            y=0.98,
        )

    # Plot 1: TPR vs FPR scatter with threshold as color
    ax = fig.add_subplot(gs[0, 0])
    ...
    # Add diagonal reference line (TPR = FPR)
    max_val = max(df["trad_fpr"].max(), 0.1)
    ax.plot([0, max_val], [0, max_val], "k--", alpha=0.3, linewidth=1)

    apply_style_to_axes(
        ax,
        title="A. TPR vs. FPR Tradeoff",
        xlabel="False Positive Rate",
        ylabel="True Positive Rate",
        paper_ready=paper_ready,
    )

    ax.set_xlim(0, max(df["trad_fpr"].max() * 1.1, 0.05))
    ax.set_ylim(0, 1.05)

    # Annotate points with high TPR and low FPR (only if not paper_ready)
    if not paper_ready:
        for _, row in df.sort_values("trad_tpr", ascending=False).head(3).iterrows():
            if row["trad_tpr"] > 0.7 and row["trad_fpr"] < 0.05:
                ax.annotate(
                    f"λ={row['threshold']}, ε={row['epsilon']}",
                    xy=(row["trad_fpr"], row["trad_tpr"]),
                    xytext=(10, 0),
                    textcoords="offset points",
                    fontsize=8,
    ...
    # Plot 2: TPR by epsilon split by betting function
    ax = fig.add_subplot(gs[0, 1])
    ...
    # Plot 3: FPR by threshold with theoretical bound
    ax = fig.add_subplot(gs[1, 0])
    ...
    # Theoretical bound
    thresholds = sorted(df["threshold"].unique())
    if thresholds and "theoretical_bound" in df.columns:
        x = np.array(thresholds)
        theoretical_values = df.groupby("threshold")["theoretical_bound"].first().values
        linewidth = 1.0 if paper_ready else 2.0
        ax.plot(
            x,
            theoretical_values,
            "k--",
            label=r"Theoretical bound ($1/\lambda$)",
            linewidth=linewidth,
            alpha=0.8,
        )

        # Plot average FPR by threshold
        for bf in sorted(df["betting_func"].unique()):
            subset = df[df["betting_func"] == bf]
            if not subset.empty:
                thr_groups = (
                    subset.groupby("threshold", observed=True)["trad_fpr"]
                    .mean()
                    .reset_index()
                )
    ...
    apply_style_to_axes(
        ax,
        title="C. Threshold vs. FPR",
        xlabel=r"Threshold ($\lambda$)",
        ylabel="False Positive Rate",
        paper_ready=paper_ready,
    )

    # Plot 4: Missed detection rate by window size
    ax = fig.add_subplot(gs[1, 1])
    ...
    if network_name is None:
        ...
        for network in df["network"].unique():
            subnet = df[df["network"] == network]
            window_groups = (
                subnet.groupby("window", observed=True)["missed_detection_rate"]
                .mean()
                .reset_index()
            )
            ...
            if not window_groups.empty:
                ...
                ax.plot(
                    window_groups["window"],
                    window_groups["missed_detection_rate"],
                    "o-",
                    label=NETWORK_NAMES.get(network, network.upper()),
                    color=NETWORK_COLORS.get(network, "gray"),
                    linewidth=linewidth,
    ...
    apply_style_to_axes(
        ax,
        title="D. Window Size vs. Missed Detections",
        xlabel="Window Size",
    ...
    if paper_ready:
        output_path = os.path.join(
            output_dir,
            f"{'comparative' if network_name is None else network_name}_detection_metrics_paper.png",
        )
    else:
        output_path = os.path.join(
            output_dir,
            f"{'comparative' if network_name is None else network_name}_detection_metrics.png",
        )
    ...
    print(f"Detection metrics tradeoff analysis saved to {output_path}")
...
def main():
    ...
    if args.network.lower() == "all":
        ...
        # Load data for all networks to create comparative epsilon plot
        networks = ["sbm", "ba", "er", "ws"]
        all_network_data = []
        ...
        for network in networks:
            ...
            if df is not None:
                ...
                all_network_data.append(df_clean)

        if all_network_data:
            combined_df = pd.concat(all_network_data, ignore_index=True)
            print("Creating comparative epsilon effect plot...")
            plot_epsilon_effect(combined_df, output_dir, paper_ready=paper_ready)

            # Add the new detection metrics plot for comparative analysis
            print("Creating comparative detection metrics tradeoff plot...")
            plot_detection_metrics_tradeoff(
                combined_df, output_dir, paper_ready=paper_ready
            )

            if generate_detailed:
                plot_epsilon_effect(combined_df, output_dir, paper_ready=False)
                plot_detection_metrics_tradeoff(
                    combined_df, output_dir, paper_ready=False
                )
        ...
        if generate_detailed:
            ...
            if "epsilon" in df_clean.columns:
                plot_epsilon_effect(
                    df_clean, output_dir, args.network, paper_ready=False
                )
            plot_detection_metrics_tradeoff(
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            ...
            if "epsilon" in df_clean.columns:
                best_epsilon = df_clean.groupby("epsilon")["trad_tpr"].mean().idxmax()
                f.write(
                    f"Best epsilon value: {best_epsilon} (avg TPR: {df_clean[df_clean['epsilon']==best_epsilon]['trad_tpr'].mean():.4f})\n"
                )
            ...
            # Add missed detection rate
            f.write(
                f"Average missed detection rate: {(1 - df_clean['trad_tpr'].mean()):.4f}\n"
            )
            ...
            f.write(
                "- Epsilon parameter affects betting aggressiveness and detection sensitivity\n"
            )
            f.write(
                "- TPR and FPR tradeoffs follow expected patterns with threshold adjustments\n"
            )
            f.write(
                "- Missed detection rates are inversely related to true positive rates\n"
            )
            ...
            f.write(
                "4. Epsilon effect analysis - showing impact of betting aggressiveness\n"
            )
            f.write(
                "5. Detection metrics tradeoff - comprehensive analysis of TPR, FPR, and missed detections\n"
            )
