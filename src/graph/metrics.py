...
from typing import Dict, List, Optional, TypeVar, Union, TypedDict, Tuple
import numpy as np
...
class FeatureDict(TypedDict, total=False):
    """Type definition for feature dictionaries."""
...
class MetricsSummary:
    ...
    @classmethod
    def from_metrics(cls, metrics: Dict[str, FeatureMetrics]) -> "MetricsSummary":
        """Create summary from feature metrics."""
        if not metrics:
            raise ValueError("Cannot create summary from empty metrics")
        ...
        ranking = [(name, m.r2) for name, m in metrics.items()]
...
def validate_inputs(func):
    ...
    def wrapper(
        ...
        **kwargs,
        ...
        return func(actual_features, predicted_features, *args, **kwargs)
...
def compute_feature_metrics(
    ...
    if n_jobs == 1:
        return _compute_metrics_sequential(
            actual_features, predicted_features, feature_names
        )
    else:
        return _compute_metrics_parallel(
            actual_features, predicted_features, feature_names, n_jobs
        )
...
def _compute_metrics_sequential(
    ...
    """Sequential computation of feature metrics."""
    metrics = {}
    ...
    with ThreadPoolExecutor(max_workers=n_jobs if n_jobs > 0 else None) as executor:
        futures = {
            feature: executor.submit(
                _compute_feature_metrics_worker,
                actual_features,
                predicted_features,
                feature,
            )
...
def _compute_feature_metrics_worker(
    actual_features: List[FeatureDict],
    predicted_features: List[FeatureDict],
    feature: str,
) -> FeatureMetrics:
    ...
    return _compute_single_feature_metrics(
        np.array(actual_values), np.array(predicted_values)
    )
...
def _compute_distribution_metrics(
    actual_tuple: Tuple[float, ...],
    predicted_tuple: Tuple[float, ...],
) -> DistributionMetrics:
    """Cached computation of distribution metrics."""
    actual = np.array(actual_tuple)
    predicted = np.array(predicted_tuple)

    # Compute histograms once for efficiency
    min_val = min(actual.min(), predicted.min())
    max_val = max(actual.max(), predicted.max())

    actual_hist, edges = np.histogram(
        actual, bins=CONFIG.hist_bins, range=(min_val, max_val), density=True
    )
    predicted_hist, _ = np.histogram(
        predicted, bins=CONFIG.hist_bins, range=(min_val, max_val), density=True
    )

    # Add small constant and normalize
    actual_hist = actual_hist + CONFIG.epsilon
    predicted_hist = predicted_hist + CONFIG.epsilon
    actual_hist = actual_hist / actual_hist.sum()
    predicted_hist = predicted_hist / predicted_hist.sum()

    return DistributionMetrics(
        kl_divergence=float(np.sum(actual_hist * np.log(actual_hist / predicted_hist))),
        js_divergence=_compute_js_from_hist(actual_hist, predicted_hist),
        wasserstein=_compute_wasserstein(actual, predicted),
        ks_statistic=_compute_ks_statistic(actual, predicted),
        mean_diff=float(np.mean(predicted) - np.mean(actual)),
        std_diff=float(np.std(predicted) - np.std(actual)),
    )


def _compute_js_from_hist(
    actual_hist: NDArray[np.float64],
    predicted_hist: NDArray[np.float64],
) -> float:
    """Compute Jensen-Shannon divergence from pre-computed histograms."""
